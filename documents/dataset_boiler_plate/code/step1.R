#' Step1.R will generate var_name.csv
#' 
#' manual: https://drexel-uhc.github.io/salurbal-project-dashboard/pages/manuals/step1.html
#' 
#' We want to look at the raw data and generate a table containing 
#'   - var_name
#'   - var_name_raw
#'   - dataset_id 

# 1. Evaluate raw data  ----------------------------------------------------------------
{
  glimpse(data_raw)
  
  #' the data is organized wide. so we will need to pull variable names
  #' 
  #' ## Recipe ##
  #'   1. select only variable names
  #'   2. put those in a table as var_name_raw
  #'   3. clean them as per manual and store then as var_name
  #'   4. Assign dataset_id
  
}

# 2. Code  ----------------------------------------------------------------
{
  # 1. select only variable names
  var_name_raw = data_raw %>% 
    select(-ISO2, -SALID1, -YEAR) %>% 
    names()
  
  # 2. put those in a table as var_name_raw
  df_var_name = tibble(var_name_raw = var_name_raw ) %>%  # step 2
    mutate(
      var_name = sanitize_codebook_var(var_name_raw), # 3. clean them as per manual and store then as var_name
      dataset_id = dataset_id # 4. Assign dataset_id
      ) 
  
}


# 3. Review and save  ----------------------------------------------------------------
{
  ## examine
  df_var_name
  
  
  ## save   
  df_var_name %>% 
    write_csv(
      get_uhc_file_path(dataset_id = dataset_id, file = "var_name.csv")
    )
}

message(paste("Step 1 Done:",dataset_id))
