...
# 1. Evaluate raw data  ----------------------------------------------------------------
{
  ...
  ## Evaluation
  ##'  it looks like we have four variables organized wide.
  ##'  - we have some strata, defintely male and female
  ##'  - teh include_... variables seem like indicators and not strata. we checked
  ##'    that each iso2, salid1, year had only one row.
}
  ...
  df_var_name = tibble(var_name_raw = vec__var_name_raw ) %>%
    mutate(
      var_name = sanitize_codebook_var(var_name_raw) %>% 
        recode("PRJPOPF" = "PRJPOP","PRJPOPM"="PRJPOP"), 
